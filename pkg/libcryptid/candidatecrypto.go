package libcryptid

import (
  "crypto"
  "crypto/rsa"
  "crypto/sha1"
  "crypto/sha256"
  "crypto/sha512"
  "crypto/aes"
  "crypto/cipher"
  "crypto/rand"
  "golang.org/x/crypto/pbkdf2"
  "io"
  "errors"
)

var CRYPTID_SALT = [...]byte{
    ,
};

//AES CRYPT
func AESEncrypt(password string, text []byte) ([]byte, error) {
		key := sha256.Sum256([]byte(password))
    pbkKey := pbkdf2.Key(key[:], CRYPTID_SALT[:], 4096, 32, sha1.New)

    block, err := aes.NewCipher(pbkKey[:])
    if err != nil {
        return nil, err
    }
    ciphertext := make([]byte, aes.BlockSize+len(text))
    iv := ciphertext[:aes.BlockSize]
    if _, err := io.ReadFull(rand.Reader, iv); err != nil {
        return nil, err
    }
    cfb := cipher.NewCFBEncrypter(block, iv)
    cfb.XORKeyStream(ciphertext[aes.BlockSize:], []byte(text))
    return ciphertext, nil
}

func AESDecrypt(password string, text []byte) ([]byte, error) {
  var key [32]byte
  key = sha256.Sum256([]byte(password))
  pbkKey := pbkdf2.Key(key[:], CRYPTID_SALT[:], 4096, 32, sha1.New)

    block, err := aes.NewCipher(pbkKey[:])
    if err != nil {
        return nil, err
    }
    if len(text) < aes.BlockSize {
        return nil, errors.New("ciphertext too short")
    }
    iv := text[:aes.BlockSize]
    text = text[aes.BlockSize:]
    cfb := cipher.NewCFBDecrypter(block, iv)
    cfb.XORKeyStream(text, text)
    return text, nil
}

//RSA Signing
func RSASign(privKey rsa.PrivateKey, msg []byte) ([]byte, error) {
		hash := sha512.New()
		hash.Write(msg)
		digest := hash.Sum(nil)

		return rsa.SignPKCS1v15(rand.Reader, &privKey, crypto.SHA512, digest)
}

func RSAVerify(pubKey rsa.PublicKey, msg []byte, sig []byte) bool {
  hash := sha512.New()
  hash.Write(msg)
  digest := hash.Sum(nil)

  err := rsa.VerifyPKCS1v15(&pubKey, crypto.SHA512, digest, sig)

  if err != nil {
    return false;
  }

  return true;
}
